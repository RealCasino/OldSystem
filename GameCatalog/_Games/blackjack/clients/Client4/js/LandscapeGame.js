...
LandscapetMain.prototype = {
    ...
    create: function() {
        ...
        var gameData = {};
        var dealBtnText, standBtnText, hitBtnText, doublesBtnText, splitBtnText, insuranceBtnText, clearAllBtnText,
            undoBtnText, newGameBtnText, repeatBtnText;
        var chip = {}, pos;

        selectedChipId = 0;
        tableItem = [
			{x:755, y:530, key:'landPlaceChips', type:'1', angle: 0},
		];
        ...
        this.buttons.deal = this.add.button(gameFrameW - 280, gameFrameH - 250, 'landGameBtnBg', this.actionDeal, this, 6, 0);
        ...
        this.dealerCardPlace = this.add.sprite(50, 240, 'landDealerCardsPlace');
        this.dealerCardPlace.width = 251;

        this.buttons.stand = this.add.button(gameFrameW - 280, gameFrameH - 250, 'landGameBtnBg', this.standBet, this, 7, 1);
        ...
        this.buttons.hit = this.add.button(gameFrameW - 800, gameFrameH - 250, 'landGameBtnBg', this.hitCard, this, 9, 3);
        ...
        this.buttons.doubles = this.add.button(gameFrame.x + 600,  gameFrameH - 250, 'landGameBtnBg', this.doubleBet, this, 8, 2);
        ...
        this.buttons.split = this.add.button(gameFrame.x + 400, gameFrameH - 250, 'landGameBtnBg', this.splitCards, this, 10, 4);
        ...
        this.buttons.insurance = this.add.button(gameFrame.x + 200, gameFrameH - 250, 'landGameBtnBg', this.insuranceBet, this, 11, 5);
        ...
        this.labels.gameInfoMsg = this.add.text(this.g_statusBg.x, this.g_statusBg.y, '',{
        ...
        this.labels.gameInfoMsg.setTextBounds(0, 0, this.g_statusBg.width, this.g_statusBg.height);
                ...
                if(gData.length > 0){
                    ...
                    for(i = 0; i < gData.length; i++){
                        if(gData[i]['type'] == 'dealer'){
                            ...
                            Global.dealerCards[0] = removableGroup.create(self.dealerCardPlace.x+5, self.dealerCardPlace.y+5, 'landCards', firstDealerCard);
                            Global.dealerCards['total_' + 0] = cardsValues[firstDealerCard];
                            Global.dealerCards[1] = removableGroup.create(
                                Global.dealerCards[0].x +  Global.dealerCards[0].width + 1,
                                Global.dealerCards[0].y,
                                'landCards',
                                54
                            );
                            ...
                            coordinates = self.getCoordinatesCard(position);
                            if(userCards.length >0){
                                ...
                                for (b = 0; b < userCards.length; b++) {
                                    if(gData[i]['isSplit']){
                                        if(pos%1 === 0){
                                            Global.userCards[pos + '_c'+count] = removableGroup.create(coordinates.x, coordinates.y - 130 + (b * 93.5) , 'landCards', self.getCards(userCards[b]));
                                        }else{
                                            Global.userCards[pos + '_c'+count] = removableGroup.create(coordinates.x + 240, coordinates.y - 130 + (b * 93.5), 'landCards', self.getCards(userCards[b]));
                                        }
                                    }else{
                                        Global.userCards[pos + '_c'+count] = removableGroup.create(coordinates.x + (b * 50), coordinates.y - 130 + (b * 46.7) , 'landCards', self.getCards(userCards[b]));
                                    }
                                    ...
                                    if( b == (userCards.length - 1)){
                                        ...
                                        self.labels.userTotal[pos] = self.add.text(Global.userCards[pos + '_c1'].x + 40, Global.userCards[pos + '_c1'].y - 35, userTotal, {
                                            font: "bold 26px ProximaNova",
                                            fill: "#ffffff"
                                        });
                                        ...
                                        if(Global.bettingChipArray[key]['type'] == position){
                                            if(gData[i].hasOwnProperty('stack')){
                                                ...
                                                if(pos%1 === 0){
                                                    ...
                                                    if(betTextArr[0] == ""){
                                                       betText.setText('x' + gData[i].stack);
                                                    } else{
                            ...
                            if(gData[i]['status'] == 'bet'){
                                ...
                                if(Global.userCards[pos + '_c1']){
                                    Global.pointer = removableGroup.create(
                                        Global.userCards[pos + '_c'+ Global.userCards['total_cards_'+ pos]].x+10,
                                        Global.userCards[pos + '_c'+ Global.userCards['total_cards_'+ pos]].y+187,
                                        'landPointer'
                                    );
                                }
	...
	getAmountDeb: function(){
         return this.chipCost[selectedChipId];
	},
    drawChip: function(par, sent){
        ...
        chip.chipClone = this.add.graphics(chipCoordinates.x + 9, chipCoordinates.y + 20, selectedChipsGroup);
        chip.active_sprite = this.add.sprite(0, 0, "chips", selectedChipId + 6);
        chip.chipClone.addChild(chip.active_sprite);

        chip.chipText = this.add.text(40, 30, '0', {
	...
	addChip:function(element){
        ...
        if(0 > Global.gameBalance - chipsAmount){
        ...
        if(!this.isEmptyObj(Global.selectedLimitArray)){
            ...
            if(Global.gameBalance - chipsAmount >= 0){
				chipsAmount = chipsAmount % 1 == 0 ? chipsAmount : parseFloat(chipsAmount);
				if(Global.bettingChipArray.length > 0){
					for (var i = 0; i < Global.bettingChipArray.length; i++) {
				...
				if (Global.selectedLimitArray[Global.limitPlateId]['Bet']['Max'] >= selectedChip.value && element.click){
					...
					chip.chipText = this.add.text(40, 30, selectedChip.value%1==0?selectedChip.value:self.roundFloat(selectedChip.value,2), {
						font: "bold 24px ProximaNova",
						fill: "#ffffff",
						wordWrap: true,
						align: "center"
	        ...
	        if (self.buttons.deal.clicked) {
	            ...
	            if (Global.bettingChipArray.length > 0) {
	                ...
	                $.client.sendPost(JSON.stringify(stake),
	                    function(msg) {
	                        var c, i, p, tI, mUC, timerVideoOfCards;
	                        if (msg.ResponseData.success) {
                                self.updateGameInfo();
	                            msgUserCards = msg.ResponseData.userCards;
	                            Global.numPlace = msgUserCards.length - 1;
	                            buttonStakeGroup.visible = false;
	                            chipsGroup.visible = false;
	                            self.buttons.stand.clicked = true;
	                            for (p = 0, tI = tableItem.length; p < tI; p++) {
	                            ...
	                            for (i = 0, mUC = msgUserCards.length; i < mUC; i++) {
	                                ...
	                                coordinates = self.getCoordinatesCard(pos);
	                                ...
	                                Global.userCards[pos + '_c1'] = removableGroup.create(coordinates.x, coordinates.y - 130, 'landCards', firstIdx);
	                                Global.userCards[pos + '_c1'].visible = false;
	                                Global.userCards[pos + '_c2'] = removableGroup.create(
	                                    Global.userCards[pos + '_c1'].x + 50,
	                                    Global.userCards[pos + '_c1'].y + Global.userCards[pos + '_c1'].height / 4,
	                                    'landCards',
	                                    secondIdx
	                                );
	                                Global.userCards[pos + '_c2'].visible = false;
	                                self.labels.userTotal[pos] = self.add.text(
	                                    Global.userCards[pos + '_c1'].x + 40,
	                                    Global.userCards[pos + '_c1'].y - 35,
	                                    total, {
	                                ...
	                                self.labels.userMsgStatus[pos + '_bg'] = self.add.sprite(
	                                    Global.userCards[pos + '_c1'].x,
	                                    Global.userCards[pos + '_c1'].y + Global.userCards[pos + '_c1'].height / 2,
	                                    'landStatusStake',
	                                    0
	                                );

	                                self.labels.userMsgStatus[pos + '_text'] = self.add.text(10, 15, '', {
	                                    font: "26px ProximaNova",
	                                    fill: "#ffffff"
	                                });
	                                self.labels.userMsgStatus[pos + '_bg'].addChild(self.labels.userMsgStatus[pos + '_text']);
	                            ...
	                            function displayElements() {
	                                ...
	                                if (Global.userCards[pos + '_c2']) {
	                                    Global.pointer = removableGroup.create(
	                                        Global.userCards[pos + '_c2'].x + 10,
	                                        Global.userCards[pos + '_c2'].y + Global.userCards[pos + '_c2'].height, 'landPointer');
	                                    Global.pointer.visible = false;
	                                    self.lightActivePosionChip(pos);
	                                }
			...
			$.client.sendPost(JSON.stringify(splite),
				function (msg) {
                    ...
                    if(msg.ResponseData.success){
                        ...
                        Global.userCards[pos + '.2_c1'] = Global.userCards[pos + '_c2'];
    	                ...
    	                tween = self.add.tween(Global.userCards[pos + '.2' + '_c1']).to({
    	                    x: Global.userCards[pos + '_c1'].x + Global.userCards[pos + '_c1'].width * 2,
    	                    y: Global.userCards[pos + '_c1'].y
    	                    }, 500, Phaser.Easing.Quadratic.Out, true, 0, 0, false
    	                );
    	                tween.onComplete.add(function(){
    	                    Global.userCards[pos + '.2_c2'] = removableGroup.create(
    	                        Global.userCards[pos + '.2_c1'].x,
    	                        Global.userCards[pos + '.2_c1'].y +  Global.userCards[pos + '.2_c1'].height / 2,
    	                        'landCards',
    	                        cardIdx2
    	                    );
                            Global.userCards[pos + '.2_c2'].visible = false;
    	                }, this);
                        c = 0;
                        timerVideoOfCards = msg.ResponseData.videoTimer;
                        function timerVideo(){
    	                ...
    	                function displaySplitCards(){
                            if(cardsVideoCount == 0){
                                ...
                                Global.userCards[pos + '.2_c2'].visible = true;
                            }
                        ...
                        function displayElements(){
                            ...
                            self.labels.userTotal[pos + 0.2] = self.add.text(
                                Global.userCards[pos + '.2_c1'].x + 40,
                                Global.userCards[pos + '.2_c1'].y - 35,
                            ...
                            self.labels.userMsgStatus[pos + 0.2 +'_bg'] = self.add.sprite(
                                Global.userCards[pos + '.2' + '_c2'].x,
                                Global.userCards[pos + '.2' + '_c2'].y + Global.userCards[pos + '.2' + '_c2'].height/2 - 20,
                                'landStatusStake',0);
                            self.labels.userMsgStatus[pos + 0.2 +'_bg'].visible = false;
                            self.labels.userMsgStatus[pos + 0.2 +'_text'] = self.add.text(10, 15, '',{
                                    font: "26px ProximaNova",
                                    fill: "#ffffff"
                                }
                            );
                            ...
                            tween = self.add.tween(Global.pointer).to({
                                x: Global.userCards[pos + '_c2'].x+10,
                                y: Global.userCards[pos + '_c2'].y + Global.userCards[pos + '_c2'].height
                                }, 500, Phaser.Easing.Quadratic.Out, true, 0, 0, false
                            );
                            tween.onComplete.add(function(){
                        ...
                        if(!!msg.ResponseData.noMoney){
                            buttonActionGroup.setAll('clicked', true);
                            self.buttons.split.visible = false;
                            self.updateGameInfo($.client.getLocalizedString('TEXT_ERROR_NO_MONEY_FOR_ACTION_MSG', true));
                            if(Global.msgTimer){
                                clearTimeout(Global.msgTimer)
                            }
                            Global.msgTimer = setTimeout(function() {
                                self.labels.gameInfoMsg.visible = false;
                    ...
                    function displayHitCard(){
                        ...
                        if(Global.splitPosition.indexOf(Global.counter) !== -1){
                            ...
                            Global.userCards[pos + '_c' + nextCards]  = removableGroup.create(
                                Global.userCards[pos + '_c' + Global.userCards['total_cards_' + pos]].x,
                                Global.userCards[pos + '_c' + Global.userCards['total_cards_' + pos]].y + Global.userCards[pos + '_c' + Global.userCards['total_cards_' + pos]].height / 2 -10,
                                'landCards',
                                cardIdx
                            );
                            ...
                            tween = self.add.tween(Global.pointer).to({
                                x: Global.userCards[pos + '_c'+ nextCards].x+10,
                                y: Global.userCards[pos + '_c'+ nextCards].y + Global.userCards[pos + '_c'+ nextCards].height
                                }, 500, Phaser.Easing.Quadratic.Out, true, 0, 0, false
                            );
                        }else{
                            pos = parseInt(Global.activedPosChips[Global.counter]);
                            ...
                            nextCards = Global.userCards['total_cards_' + pos] + 1;
                            Global.userCards[pos + '_c' + nextCards]  = removableGroup.create(
                                Global.userCards[pos + '_c' + Global.userCards['total_cards_' + pos]].x + 50,
                                Global.userCards[pos + '_c' + Global.userCards['total_cards_' + pos]].y + Global.userCards[pos + '_c' + Global.userCards['total_cards_' + pos]].height/4,
                                'landCards',
                                cardIdx
                            );
                            Global.userCards['total_cards_' + pos] = nextCards;
                ...
                if(msg.ResponseData.success){
    				...
    				function displayDouble(){
    	                ...
    	                if(Global.splitPosition.indexOf(Global.counter) !== -1){
                            ...
                            Global.userCards[pos + '_c' + nextCards]  = removableGroup.create(
                                Global.userCards[pos + '_c' + Global.userCards['total_cards_' + pos]].x,
                                Global.userCards[pos + '_c' + Global.userCards['total_cards_' + pos]].y + Global.userCards[pos + '_c' + Global.userCards['total_cards_' + pos]].width / 2,
                                'landCards',
                                cardIdx
                            );
                            tween = self.add.tween(Global.pointer).to({
	...
	standBet: function(element, isAuto){
		var self = this;
        var userTotal, userTotal2, insurance = 0, pos, pos2, splitPosItem = 0, stand = {};
        if(isAuto === undefined){
            isAuto = false;
        ...
        if ((element && element.clicked) || Boolean(isAuto)){
            ...
            if((Global.splitPosition.indexOf(Global.counter) !== -1 ) && Global.splitPositionItem == 0){
                ...
                pos2 = Global.splitPositionItem + 0.2;
                userTotal2 = Global.userCards['total_'+ pos2];
                ...
                self.add.tween(Global.pointer).to({
                    x: Global.userCards[pos + '.2_c2'].x + 10,
                    y: Global.userCards[pos + '.2_c2'].y + Global.userCards[pos + '.2_c2'].height
                    }, 500, Phaser.Easing.Quadratic.Out, true, 0, 0, false
                );
                self.lightActivePosionChip(pos);
                if(userTotal == 10 || userTotal == 11 || userTotal2 == 10 || userTotal2 == 11){
                    self.buttons.doubles.visible = true;
                }else{
                    self.buttons.doubles.visible = false;
                }
            ...
            }else if(Global.counter < Global.numPlace){
                Global.counter +=1;
                stand.type = 'stand';
                ...
                self.lightActivePosionChip(parseInt(pos, 10));
                if( (Global.splitPosition.indexOf(Global.counter) == -1) && (Global.userCards['point1_' + pos] == Global.userCards['point2_' + pos])){
                   self.buttons.split.visible = true;
                }else{
                    self.buttons.split.visible = false;
				...
				function (msg) {
                    ...
                    if(msg.ResponseData.success){
					   //console.log('insurance_msg', msg);
                        ...
                        text = $.client.getLocalizedString('TEXT_INSURANCE_ADDED', true);
                        self.g_statusBg.loadTexture('statusBg', 1);
                        self.labels.gameInfoMsg.setText(text);
                        self.labels.gameInfoMsg.visible = true;
                        ...
                        sT = setTimeout(function() {
                            self.g_statusBg.loadTexture('statusBg', 2);
                            self.labels.gameInfoMsg.visible = false;
                            self.g_statusBg.visible = false;
                        }, 4000);
	...
	showDealerCards: function(){
		...
		$.client.sendPost(JSON.stringify(getDealerCard),
			function (msg) {
				...
				Global.dealerCards[1].loadTexture('landCards', secondDealerCard);
				...
				if(msg.ResponseData.cards.length > 1){
					for (var i = 1; i < msg.ResponseData.cards.length; i++) {
						...
						Global.dealerCards[Global.localCounter + 1] = removableGroup.create(
							Global.dealerCards[Global.localCounter].x + Global.dealerCards[Global.localCounter].width + 5,
                ...
                function displayElements(){
                    ...
                    $.client.sendPost(JSON.stringify(stake),
                        function (msg) {
                            ...
                            if(msg.ResponseData['stake'].length > 0){
                                for (var i = 0; i < msg.ResponseData['stake'].length; i++) {
                                    ...
                                    if(msg.ResponseData.stake[i]['status'] == 'win'){
                                        ...
                                        sum += msg.ResponseData.stake[i]['gainings'];
                            ...
                            if(sum > 0){
                                text = $.client.getLocalizedString('TEXT_WON_SUM', true) + sum;
                                self.g_statusBg.loadTexture('statusBg', 0);
                                self.labels.gameInfoMsg.setText(text);
                                self.labels.gameInfoMsg.visible = true;
                                self.g_statusBg.visible = true;
                            }
	...
	repeatGame: function(){
        ...
        for(var key2 in Global.bettingChipArray){
            if(Global.bettingChipArray.hasOwnProperty(key2)){
                console.log('total', Global.bettingChipArray[key2]['total']);
                amount = Global.bettingChipArray[key2]['total'];
                amount = amount.toFixed(1);
                Global.bettingChipArray[key2]['chip']['chipText'].setText(amount);
                Global.bettingChipArray[key2]['sent'] = false;
            }
        }

        Global.betAmount = parseFloat(amount);
	...
	resetValues: function(){
        ...
        this.buttons.split.visible = false;
        this.buttons.insurance.visible = false;
        this.g_statusBg.loadTexture('statusBg', 2);
        this.labels.gameInfoMsg.visible = false;
        this.g_statusBg.visible = false;
	},
	...
	updateMoneyInfo: function() {
        if(typeof this.labels.userBalance.setText === 'function'){
            this.labels.userBalance.setText($.client.UserData.CurrencySign + parseFloat(Global.gameBalance).toFixed(2));
        }
    },
    updateUserInfoText: function(id, text, status){
    	...
    	if(status == 'win'){
			this.labels.userMsgStatus[id+'_bg'].loadTexture('landStatusStake', 1);
    	}else if(status == 'push'){
    		this.labels.userMsgStatus[id+'_bg'].loadTexture('landStatusStake', 2);
    	}else {

        }
    	this.labels.userMsgStatus[id+'_text'].setText(text);
    	this.labels.userMsgStatus[id+'_bg'].visible = true;
    	removableGroup.bringToTop(this.labels.userMsgStatus[id+'_bg']);
    },
    ...
    updateGameInfo: function(text){
    	if(text !== undefined){
            ...
            if(text.length > 0){
                this.g_statusBg.visible = true;
                this.labels.gameInfoMsg.visible = true;
            } else{
                this.g_statusBg.visible = false;
                this.labels.gameInfoMsg.visible = false;
            }
        ...
        if (!Global.isModalShow) {
            if (Global.selectedLimitArray.length != 0) {
                ...
                for (var i = start; i < end; i++) {
                	...
                	var minLbl = limitBtn.addChild(self.add.text(85, 15, min, {
