...
var dispatcher = function messageDispatcher(sessionId, message) {
    ...
    var userSettings, selectedLimit = {}, timerVideo;

    var gameId, videoOfCards=[], timerVideoOfCards = [], counter =0;
    var cancelResult, resultCode, interimBets;
    ...
    switch (message.type) {
        case "get_data":{
            ...
            return {success: true, limits: context.gameConfiguration.Limits, selectedLimit: selectedLimit, game: filteredData};
        }
        case "refresh_user_data":
                ...
                if(validateBet(sessionId, message.bets[0].amount,  message.bets[0].pos)){
                    ...
                    if(message.bets.length>0){
                        message.bets.forEach(function (bet) {
                            ...
                            interimBets.roundBetId = betResult;
                            gBets.push(interimBets);
        ...
        case "deal": {
            videoOfCards = [];
            if(typeof shuffledMaps[gameId] === 'undefined'){
                shuffledMaps[gameId] = getRng(sessionId).shuffle(cards);
            }
            ...
            if(context.mediaHelper.getVODInfo('/Part02/' + dealerCard)){
            ...
            try{
                ...
                for(var d = 0; d < dealData.length; d++){
                    ...
                    if(context.mediaHelper.getVODInfo('/Part02/' + userCard)){
                ...
                for (var i = 0; i < userCards.length; i++) {
                    ...
                    if(context.mediaHelper.getVODInfo('/Part02/' + userCards[i].second)){
                ...
                timerVideo = function (){
                    ...
                    if(counter < videoOfCards.length){
                        if(context.mediaHelper.getVODInfo(videoOfCards[counter-1].name)){
        ...
        case "hit":
            ...
            if (tmpUserCards.length > 0){
                if(tmpUserCards[0]['points'] < 21) {
                    ...
                    if(context.mediaHelper.getVODInfo('/Part02/' + otherOneCard)){
        ...
        case "split":
            ...
            if(filteredUserCards.length >0) {
                if (filteredUserCards[0]['first'] == filteredUserCards[0]['second'] && alredySplired.length === 0) {
                    ...
                    if(context.mediaHelper.getVODInfo('/Part02/' + userCard2)){
                    ...
                    sum = parseFloat(filteredUserCards[0]['stack']);
                    if(user.balance - (sum + sum) >= 0) {
                        tmpUserCards.push({
                            sessionId: sessionId,
                        ...
                        for (var i = 0; i < filteredData.length; i++) {
                            if(filteredData[i].type == 'status'){
                                ...
                                if(filteredData[i].pos == filteredUserCards[0]['pos']){
                                    tmpUserCards2 = filteredData[i].cards;
                                    tmpUserCards2 = tmpUserCards2.split(',');
                                    filteredData[i].cards = tmpUserCards2[0] +','+userCard;
                        ...
                        gameData.push({
                            ...
                            pos: parseInt(filteredUserCards[0]['pos'], 10) + 0.2,
                            cards: tmpUserCards2[1] + ',' + userCard2,
                            isSplit: true,
                        ...
                        timerVideo();
                        try{
                            var betResult = context.accountingHelper.registerBet(sessionId, settings.roundId, sum, betObj, "Split cards at round " + settings.roundId);
                            switch (betResult) {
                                case betErrorCodes.FATAL_ERROR: {
                                    throw "FATAL_ERROR";
                                }
        ...
        case "double":
            ...
            if(tmpUserCards[0]['points'] === 10 || tmpUserCards[0]['points'] === 11 ){
                otherOneCard = getCard(gameId);
                points = cardsValues[cards.indexOf(otherOneCard)];
                videoOfCards.push({name: '/Part02/' + otherOneCard});
                if(context.mediaHelper.getVODInfo('/Part02/' + otherOneCard)){
                ...
                tmpUserCards[0]['points'] += points;
                sum = parseFloat(tmpUserCards[0]['stack']);
                ...
                if(user.balance - sum > 0){
                    ...
                    timerVideo();
                    try{
                        var betResult = context.accountingHelper.registerBet(sessionId, settings.roundId, sum, betData, "Double bet at round " + settings.roundId);
                        switch (betResult) {
                            case betErrorCodes.FATAL_ERROR: {
                                throw "FATAL_ERROR";
                            }
       ...
       case "insurance":
            ...
            if(filteredDealerCards[0]['points'] == 11){
                ...
                if(user.balance - sum >= 0) {
                    ...
                    try{
                        var betResult = context.accountingHelper.registerBet(sessionId, settings.roundId, tmpUserCards[0]['insurance'], { type: 'insurance' }, "Insurance bet at round " + settings.roundId);
                        switch (betResult) {
                            case betErrorCodes.FATAL_ERROR: {
                                throw "FATAL_ERROR";
                            }
        ...
        case "stake":{
            ...
            try {
                filteredBet = gBets.filter(function (bet) {
                    return bet.sessionId === sessionId && bet.pos == stakeUserCards.pos;
                });
                 if (hasWon) {
                    for (var i = 0; i < filteredBet.length; i++) {
                        winAmount =  parseFloat(filteredBet[i].value + (filteredBet[i].value * winRate) + sum);
                        betData = {
                            sessionId: sessionId,
                            pos: stakeUserCards.pos,
                            amount: filteredBet[i].value
                        };

                        resultCode = context.accountingHelper.registerWin(sessionId, settings.roundId,filteredBet[i].roundBetId, winAmount, betData, "Won = " + winAmount + ' at round ' + settings.roundId,
                            function() {
                                if (user != null) {
                                    notifyUser(sessionId, {
                                        success: true,
                    ...
                    for (var i = 0; i < filteredBet.length; i++) {
                        betData = {
                            sessionId: sessionId,
                            pos: filteredBet[i].pos,
                            amount: filteredBet[i].value
                        };
                        resultCode = context.accountingHelper.registerWin(sessionId, settings.roundId, filteredBet[i].roundBetId, betData.amount,betData, "Push = " + sum + 'at round ' + settings.roundId,
                            function() {
                                if (user != null) {
                                    notifyUser(sessionId, {
                                        success: true,
                    ...
                    if (insuranceSum > 0) {
                        for (var i = 0; i < filteredBet.length; i++) {
                            if (filteredBet[i].insurance) {
                                resultCode = context.accountingHelper.registerWin(sessionId, settings.roundId, filteredBet[i].roundBetId, insuranceSum, { type: 'insurance' }, "Insurance = " + insuranceSum + 'at round ' + settings.roundId,
                                    function() {
                                        if (user != null) {
                                            notifyUser(sessionId, {
                                                success: true,
            ...
            runtime.setGameState(sessionId, "WINNING", "Winner: " + hasWon);
            context.accountingHelper.registerRoundResult(settings.roundId, hasWon);
