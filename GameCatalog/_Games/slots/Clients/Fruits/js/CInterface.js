function CInterface(iCurBet,iTotBet,iMoney){
    ...
    this._init = function(iCurBet,iTotBet,iMoney){
        ...
        _pStartPosExit = {x:CANVAS_WIDTH - (oSprite.width/2) - 50,y:(oSprite.height/2) + 2};


        if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
            _pStartPosAudio = {x:_pStartPosExit.x - oSprite.width,y:(oSprite.height/2) + 2};
            ...
            _pStartPosFullscreen = {x:_pStartPosExit.x - oSprite.width,y:(oSprite.height/2) + 2};
        }
        ...
        _oSpinBut = new CTextButton(1026 + (oSprite.width/2),595 ,oSprite,$.client.getLocalizedString("TEXT_SPIN", true),FONT_GAME,"#ffffff",30);
        _oSpinBut.addEventListener(ON_MOUSE_UP, this._onSpin, this);

        oSprite = s_oSpriteLibrary.getSprite('info_but');
        _oInfoBut = new CTextButton(296 + (oSprite.width/2),595,oSprite,$.client.getLocalizedString("TEXT_INFO", true),FONT_GAME,"#ffffff",30);
        _oInfoBut.addEventListener(ON_MOUSE_UP, this._onInfo, this);

        oSprite = s_oSpriteLibrary.getSprite('but_lines_bg');
        _oAddLineBut = new CTextButton(436 + (oSprite.width/2),595,oSprite,$.client.getLocalizedString("TEXT_LINES", true),FONT_GAME,"#ffffff",30);
        _oAddLineBut.addEventListener(ON_MOUSE_UP, this._onAddLine, this);

        oSprite = s_oSpriteLibrary.getSprite('coin_but');
        _oBetCoinBut = new CTextButton(620 + (oSprite.width/2),595,oSprite,$.client.getLocalizedString("TEXT_COIN", true),FONT_GAME,"#ffffff",30);
        _oBetCoinBut.addEventListener(ON_MOUSE_UP, this._onBet, this);

        oSprite = s_oSpriteLibrary.getSprite('but_maxbet_bg');
        _oMaxBetBut = new CTextButton(805 + (oSprite.width/2),595,oSprite,$.client.getLocalizedString("TEXT_MAX_BET", true),FONT_GAME,"#ffffff",30);
        _oMaxBetBut.addEventListener(ON_MOUSE_UP, this._onMaxBet, this);

        _oMoneyText = new createjs.Text($.client.getLocalizedString(USER_NAME, true) +"\n"+ iMoney + $.client.UserData.CurrencySign,"24px "+FONT_GAME, "#ffde00");
        _oMoneyText.x = 408;
        _oMoneyText.y = 14;
        ...
        _oTotalBetText = new createjs.Text($.client.getLocalizedString("TEXT_BET", true) +": "+iTotBet.toFixed(2),"30px "+FONT_GAME, "#ffffff");
    ...
    this.refreshMoney = function(iMoney){
        _oMoneyText.text = USER_NAME +"\n"+iMoney.toFixed(2)+$.client.UserData.CurrencySign;
    };
    ...
    this.refreshTotalBet = function(iTotBet){
        _oTotalBetText.text = $.client.getLocalizedString("TEXT_BET", true) +": "+iTotBet.toFixed(2);
    };
    ...
    this.refreshWinText = function(iWin){
        _oWinText.text = $.client.getLocalizedString("TEXT_WIN", true) + " "+iWin.toFixed(2)+$.client.UserData.CurrencySign;
    };
